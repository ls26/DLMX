...
def comp_class_vec(ouput_vec, index=None):
    ...
    one_hot = torch.zeros(1, ouput_vec.shape[1]).scatter_(1, index, 1).cuda()
...
if __name__ == '__main__':
    ...
    df = pd.read_excel('/mnt/sda/fengwei/AMD_code/Training400/adam_data.xlsx',
                       index_col='ID')
    output_dir = "backward_hook_cam_mtl_{}".format(args.arc)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    task_groups = [{'ids': 0,
                    'type': 'classif',
                    'size': 1}]
    if args.arc=="efficient":
        net = get_efficientunet_b3(out_channels=2, concat_input=True, pretrained=True).cuda()
    elif args.arc=="deeplab":
        #model = MTL_UNet_v2(opt).to(device)
        net = DeepLab(backbone='resnet', output_stride=16).cuda()
    elif args.arc=="vgg":
        #model = ResUnet(channel=3).cuda()
        net = get_efficientunet_b0(out_channels=2, concat_input=True, pretrained=True).cuda()

    #net = get_efficientunet_b3(out_channels=2, concat_input=True, pretrained=False).cuda()
    classes = ('Non AMD','AMD')
    # Saving settings
    model_dir = args.checkpoint_path
    path_net = os.path.join(model_dir, 'AMD_MTL_CAM_{}.pkl'.format(args.arc))
    ...
    for idx_ in df.index:
        ...
        # 注册hook
        net.encoder.head_conv.register_forward_hook(farward_hook)
        net.encoder.head_conv.register_backward_hook(backward_hook)
        # forward
        output = net(img_input.cuda())
    
        output_soft = F.softmax(output[0], dim=1)
        # print(output_soft,output_soft.max(dim=1)[0])
        if np.max(output_soft[0].cpu().data.numpy())>0.5:
            idx = np.argmax(output[0].cpu().data.numpy())
            print("predict: {}".format(classes[idx]))

            # backward
            net.zero_grad()
            class_loss = comp_class_vec(output[0])
