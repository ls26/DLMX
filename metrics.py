...
class RefugeMetrics():
    ...
    def __init__(self, task_groups, opt):
        ...
        self.add_metric('auc_ppa', True, reduce_type=['auc'])
    ...
    def link_metric(self, source_metric, target_metrics, link_type='mean'):
        self.metrics[self.metrics_names.index(source_metric)].link([self.metrics[self.metrics_names.index(elt)]
                                                                    for elt in target_metrics], link_type)
    ...
    def incr(self, preds, gts, pred_vCDR, gt_vCDR):
        ...
        self.incr_metric('auc_ppa', preds[-2].cpu().numpy().tolist(), gts[-1].cpu().numpy().tolist())
