...
if opt.arc=="efficient":
    model = get_efficientunet_b3(out_channels=2, concat_input=True, pretrained=False).cuda()
...
pretrained_model_pth = os.path.join(model_dir,'AMD_MTL_{}.pkl'.format(opt.arc))
...
if not osp.exists("./{}/save_results/lesion/".format(opt.arc) + "train"):
    os.makedirs("./{}/save_results/lesion/".format(opt.arc) + "train")

with open(osp.join("./{}/save_results/lesion/".format(opt.arc) + "train",'train_log.csv'), 'w') as f:
    log = [ ["img_name"] +['AMD classification probability'] +["AMD classification prediction"]+
           ["AMD Classification GT"]+["dice drusen"]+["dice exudate"]+["dice hemorrhage"]+["dice others"]+["dice scar"]
...
for file_one in train_feature_list:
    ...
    save_img_path = "./{}/save_results/lesion/".format(opt.arc) + "train" + "/img/"
    save_gt_path = "./{}/save_results/lesion/".format(opt.arc) + "train" + "/gt/"
    save_pred_path = "./{}/save_results/lesion/".format(opt.arc) + "train" + "/pred/"

    save_disc_gt_path = "./{}/save_results/lesion/".format(opt.arc) + "train" + "/disc_gt/"
    save_disc_pred_path = "./{}/save_results/lesion/".format(opt.arc) + "train" + "/disc_pred/"
    ...
    with torch.no_grad():
        ...
        pred_b_main_soft = model(img.cuda())

        pred_b_main = torch.argmax(pred_b_main_soft[2], dim=1)

        pred_img_all.append(pred_b_main_soft[2])
        label_img_all.append(transforms.functional.to_tensor(mask_all).type(torch.LongTensor).cuda())
        
        _, dice_arr_one, trg_class_number_one,pred_class_number_one = dice_eval(pred=pred_b_main_soft[2], label=transforms.functional.to_tensor(mask_all).type(torch.LongTensor).cuda(),
                                              n_class=6)
        ...
        disc_pred_main = (pred_b_main_soft[1].cpu().data.numpy() >= 0.5).astype(np.float32)
        ...
        with open(osp.join("./{}/save_results/lesion/".format(opt.arc) + "train", 'train_log.csv'), 'a') as f:
            # print(vCDR_error)
            log = [img_name,round(F.softmax(pred_b_main_soft[0], dim=1)[:, 1].cpu().data.numpy()[0],4),
                   int((F.softmax(pred_b_main_soft[0], dim=1)[:, 1].cpu().data.numpy()[0] >= 0.5)),
                  img_label,dice_arr_one[1],dice_arr_one[2],dice_arr_one[3],dice_arr_one[4],dice_arr_one[5],
...
pred_img_all = []
label_img_all = []
if not osp.exists("./{}/save_results/lesion/".format(opt.arc) + "test"):
    os.makedirs("./{}/save_results/lesion/".format(opt.arc) + "test")
with open(osp.join("./{}/save_results/lesion/".format(opt.arc) + "test",'test_log.csv'), 'w') as f:
    log = [ ["img_name"] +['AMD classification probability'] +["AMD classification prediction"]+
           ["AMD Classification GT"]+["dice drusen"]+["dice exudate"]+["dice hemorrhage"]+["dice others"]+["dice scar"]
                      +["area drusen"]+["area exudate"]+["area hemorrhage"]+["area others"]+["area scar"]
           +["area pred drusen"]+["area pred exudate"]+["area pred hemorrhage"]+["area pred others"]+["area pred scar"]]
...
for file_one in test_feature_list:
    ...
    scar_path = file_one[11]
    # print(img_name,img_label,disc_path,drusen_path,exudate_path)
    save_img_path = "./{}/save_results/lesion/".format(opt.arc) + "test" + "/img/"
    save_gt_path = "./{}/save_results/lesion/".format(opt.arc) + "test" + "/gt/"
    save_pred_path = "./{}/save_results/lesion/".format(opt.arc) + "test" + "/pred/"

    save_disc_gt_path = "./{}/save_results/lesion/".format(opt.arc) + "test" + "/disc_gt/"
    save_disc_pred_path = "./{}/save_results/lesion/".format(opt.arc) + "test" + "/disc_pred/"
    if not os.path.exists(save_img_path):
        os.makedirs(save_img_path)
    if not os.path.exists(save_gt_path):
        os.makedirs(save_gt_path)
    ...
    with torch.no_grad():
        ...
        with open(osp.join("./{}/save_results/lesion/".format(opt.arc) + "test", 'test_log.csv'), 'a') as f:
            # print(vCDR_error)
            log = [img_name,F.softmax(pred_b_main_soft[0], dim=1)[:, 1].cpu().data.numpy()[0],
                   int((F.softmax(pred_b_main_soft[0], dim=1)[:, 1].cpu().data.numpy()[0] >= 0.5)),
                  img_label,dice_arr_one[1],dice_arr_one[2],dice_arr_one[3],dice_arr_one[4],dice_arr_one[5],
                  trg_class_number_one[1],trg_class_number_one[2],trg_class_number_one[3],trg_class_number_one[4],trg_class_number_one[5]
                  ,pred_class_number_one[1],pred_class_number_one[2],pred_class_number_one[3],pred_class_number_one[4],pred_class_number_one[5]
...
print('Test dice arr is {}'.format(dice_arr.shape))
print(pred_img_all.shape,label_img_all.shape)
print('Test Dice:')
print(dice_arr,trg_class_number,pred_class_number)
print('dsc_drusen :%.1f' % (dice_arr[1]))
