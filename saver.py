...
class Saver():
        ...
        if args:
            ...
            if args.method.upper() == 'STL':
                ...
                if self.active_task!=3:
                    self.best_error = 0.
            elif args.method.upper() == 'MTL':
                self.best_error = 0.
        self.config_file = os.path.join(self.save_dir,'config.json')
        self.best_error_weights_file = os.path.join(self.save_dir,'best_error_weights')
        self.metrics = []
    ...
    def checkpoint(self, model, epoch, n_iter, iter_dict, optimizer=None):
        ...
        if self.STL:
            if self.active_task==0:
                ...
                if iter_dict[ref_met]['val']['reduced'] > self.best_error:
                    print('Best error.')
                    torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_clf"+".pth")
                ...
                if iter_dict[ref_met]['val']['reduced'] > self.best_error:
                    print('Best error.')
                    torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_od"+".pth")
                    self.best_error = iter_dict[ref_met]['val']['reduced']
                    self.config_dict['best_error'] = self.best_error
                ...
                if iter_dict[ref_met]['val']['reduced'] > self.best_error:
                    print('Best error.')
                    torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_oc"+".pth")
                    self.best_error = iter_dict[ref_met]['val']['reduced']
                    self.config_dict['best_error'] = self.best_error
                ...
                if iter_dict[ref_met]['val']['reduced'] < self.best_error:
                    print('Best error.')
                    torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_fov"+".pth")
                    self.best_error = iter_dict[ref_met]['val']['reduced']
                    self.config_dict['best_error'] = self.best_error
            elif self.active_task == 4:
                ref_met = 'auc_ppa'
                if iter_dict[ref_met]['val']['reduced'] > self.best_error:
                    print('Best error.')
                    torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_ppa"+".pth")
                    self.best_error = iter_dict[ref_met]['val']['reduced']
                    self.config_dict['best_error'] = self.best_error
        else:
            ref_met_4 = 'auc_ppa'
            ...
            ref_met_1 = 'dsc_od'
            ref_met_2 = 'dsc_oc'
            auc_all = iter_dict[ref_met_0]['val']['reduced']+iter_dict[ref_met_1]['val']['reduced']+\
                      iter_dict[ref_met_2]['val']['reduced']+iter_dict[ref_met_4]['val']['reduced']
            if auc_all> self.best_error:
            # if iter_dict['loss']['val'] < self.best_error:
            #     print('Best error.')
            #     print( self.best_error_weights_file+"_Glaucoma_MTL"+".pth")
                torch.save(ckpt, self.best_error_weights_file+"_Glaucoma_MTL"+".pth")
                self.best_error = auc_all
            # iter_dict['loss']['val']
                self.config_dict['best_error'] = self.best_error
        ...
        torch.save(ckpt, os.path.join(self.save_dir, 'model_weights.pth'))
